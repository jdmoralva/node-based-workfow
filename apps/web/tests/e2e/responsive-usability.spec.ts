...
test.describe("responsive usability details", () => {
  for (const [viewportLabel, viewport] of viewportCases) {
    test(`landing keeps hero and application cards readable at ${viewportLabel}`, async ({ page }) => {
    ...
    test(`applications keep a deliberate grid at ${viewportLabel}`, async ({ page }) => {
      await page.setViewportSize(viewport);
    ...
    test(`services keep toolbar actions and readable card flow at ${viewportLabel}`, async ({ page }) => {
      await page.setViewportSize(viewport);
    ...
    test(`creditmodeler keeps stage, tree, and canvas usable at ${viewportLabel}`, async ({ page }) => {
      await page.setViewportSize(viewport);
